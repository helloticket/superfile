...
type remessaAEJFile struct {
	model    *model.Remessa
	fileName string
	encoder  *Encoder
}

func (w *remessaAEJFile) Write() *os.File {
...
func (w *remessaAEJFile) encodeLotes() []string {
	encoded := []string{}
...
func (w *remessaAEJFile) encodeFileHeader() string {
...
func (w *remessaAEJFile) encodeFileTrailer() string {
	config := w.model.GetRemessaLayout()
...
func (w *remessaAEJFile) getLayoutFor(name string) map[interface{}]interface{} {
	config := w.model.GetRemessaLayout()
