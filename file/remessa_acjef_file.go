...
type remessaACJEFFile struct {
	model    *model.Remessa
	fileName string
	encoder  *Encoder
}

func (w *remessaACJEFFile) Write() *os.File {
...
func (w *remessaACJEFFile) encodeLotes() []string {
	encoded := []string{}
...
func (w *remessaACJEFFile) encodeFileHeader() string {
...
func (w *remessaACJEFFile) encodeFileTrailer() string {
	config := w.model.GetRemessaLayout()
...
func (w *remessaACJEFFile) getLayoutFor(name string) map[interface{}]interface{} {
	config := w.model.GetRemessaLayout()
